...
Thanks for helping improve Stylet. Bug reports, focused fixes, documentation improvements, and well-scoped features are welcome.
...
- Search [existing issues](https://github.com/focale-editor/stylet/issues) before opening a new one.
...
Stylet follows the SDK constraint in [`pubspec.yaml`](pubspec.yaml). Install a compatible stable Flutter SDK, fork the repository, and run:

```sh
git clone https://github.com/YOUR_ACCOUNT/stylet.git
cd stylet
flutter pub get
```
...
For backend changes, also follow the contracts and platform notes under [docs/](docs/), then exercise the affected device or platform when possible.
