...
The method channel accepts `getCapabilities` with no arguments. A successful
response is a list of names from `StylusFeature`. It also accepts
`setTabletPadOverrideEnabled` with a Boolean `enabled` argument. That method
returns whether a compatible vendor control accepted the requested state;
backends whose pad input is passive return `false`. Unknown methods must return
Flutter's not-implemented response.
...
Pen-motion observers must remain passive. Android and desktop callbacks return
the source event unchanged, and iOS recognizers must not cancel touches or
claim a gesture Flutter would otherwise receive. Vendor tablet-pad mappings are
the sole exception and may be claimed only after the explicit method above;
they must be released on opt-out, stream cancellation, and backend teardown.
...
| `preferredAction` | string  | Optional system preference: `ignore`, `switchEraser`, `switchPrevious`, `showColorPalette`, `showInkAttributes`, `showContextualPalette`, `runSystemShortcut`, or `unknown`. |
